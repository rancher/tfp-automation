...
func (p *UpgradeImportedClusterTestSuite) TestTfpUpgradeImportedCluster() {
	...
	for _, tt := range tests {
		...
		p.T().Run(tt.name, func(t *testing.T) {
			...
			clusters, _ := provisioning.Provision(p.T(), p.client, p.standardUserClient, rancher, terraform, terratest, testUser, testPassword, perTestTerraformOptions, configMap, newFile, rootBody, file, false, false, true, clusterIDs, nil, nestedRancherModuleDir)
			...
			clusterResp, err := p.client.Management.Cluster.ByID(clusters[0].ID)
			require.NoError(p.T(), err)

			cluster, err := p.client.Steve.SteveType(stevetypes.Provisioning).ByID(namespaces.FleetDefault + "/" + clusterResp.ID)
			require.NoError(p.T(), err)
