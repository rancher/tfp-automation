...
func snapshotRestore(t *testing.T, client *rancher.Client, rancherConfig *rancher.Config, terraformConfig *config.TerraformConfig,
	...
	cluster, snapshotName, postDeploymentResp, postServiceResp, err := snapshotV2Prov(t, client, rancherConfig, terraformConfig, terratestConfig, podTemplate, testUser, testPassword, clusterName, poolName, clusterID, terraformOptions)
	require.NoError(t, err)

	restoreV2Prov(t, client, rancherConfig, terraformConfig, terratestConfig, snapshotName, testUser, testPassword, clusterName, poolName, cluster, clusterID, terraformOptions)
...
func snapshotV2Prov(t *testing.T, client *rancher.Client, rancherConfig *rancher.Config, terraformConfig *config.TerraformConfig,
	...
	_, err := framework.ConfigTF(nil, rancherConfig, terraformConfig, terratestConfig, testUser, testPassword, clusterName, poolName, "", nil)
	require.NoError(t, err)
	...
	if terratestConfig.SnapshotInput.SnapshotRestore == kubernetesVersion || terratestConfig.SnapshotInput.SnapshotRestore == all {
		upgradeCluster(t, client, rancherConfig, clusterName, poolName, testUser, testPassword, clusterID, terratestConfig, terraformConfig, terraformOptions)
	}
...
func upgradeCluster(t *testing.T, client *rancher.Client, rancherConfig *rancher.Config, clusterName, poolName, testUser, testPassword,
	clusterID string, terratestConfig *config.TerratestConfig, terraformConfig *config.TerraformConfig, terraformOptions *terraform.Options) {
	clusterObject, _, err := clusters.GetProvisioningClusterByName(client, clusterName, namespace)
	require.NoError(t, err)
