...
func (k *KubernetesUpgradeTestSuite) TestTfpKubernetesUpgrade() {
	...
	for _, tt := range tests {
		...
		k.Run((tt.name), func() {
			...
			clusterIDs, _ := provisioning.Provision(k.T(), k.standardUserClient, rancher, terraform, terratest, testUser, testPassword, k.terraformOptions, configMap, newFile, rootBody, file, false, false, false, nil)
			provisioning.VerifyClustersState(k.T(), adminClient, clusterIDs)

			provisioning.KubernetesUpgrade(k.T(), k.standardUserClient, rancher, terraform, terratest, testUser, testPassword, k.terraformOptions, configMap, newFile, rootBody, file, false)
			provisioning.VerifyClustersState(k.T(), adminClient, clusterIDs)
		})
