...
func (s *TfpSanityRKE1ProvisioningTestSuite) TestTfpRKE1ProvisioningSanity() {
	...
	for _, tt := range tests {
		...
		s.Run((tt.name), func() {
			...
			clusterIDs, _ := provisioning.Provision(s.T(), s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, s.terraformOptions, configMap, newFile, rootBody, file, false, false, true, customClusterNames)
