...
type TfpSanityIPv6UpgradeRancherTestSuite struct {
	suite.Suite
...
func (s *TfpSanityIPv6UpgradeRancherTestSuite) TearDownSuite() {
...
func (s *TfpSanityIPv6UpgradeRancherTestSuite) TestTfpUpgradeIPv6Rancher() {
	...
	allClusterIDs := s.provisionAndVerifyCluster("Sanity_IPv6_Pre_Rancher_Upgrade_", clusterIDs, newFile, rootBody, file, standardUserClient, standardToken, testUser, testPassword)

	s.client, s.cattleConfig, s.terraformOptions, s.upgradeTerraformOptions = ranchers.UpgradeIPv6Rancher(s.T(), s.client, s.serverNodeOne, s.session, s.cattleConfig)
	...
	s.rancherConfig, s.terraformConfig, s.terratestConfig, _ = config.LoadTFPConfigs(s.cattleConfig)
	s.provisionAndVerifyCluster("Sanity_IPv6_Post_Rancher_Upgrade_", nil, newFile, rootBody, file, standardUserClient, standardToken, testUser, testPassword)
...
func (s *TfpSanityIPv6UpgradeRancherTestSuite) provisionAndVerifyCluster(name string, clusterIDs []string, newFile *hclwrite.File, rootBody *hclwrite.Body,
...
func TestTfpSanityIPv6UpgradeRancherTestSuite(t *testing.T) {
	suite.Run(t, new(TfpSanityIPv6UpgradeRancherTestSuite))
}
