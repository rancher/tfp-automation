...
type TfpSanityDualStackProvisioningTestSuite struct {
	suite.Suite
...
func (s *TfpSanityDualStackProvisioningTestSuite) TearDownSuite() {
	_, keyPath := rancher2.SetKeyPath(keypath.DualStackKeyPath, s.terratestConfig.PathToRepo, s.terraformConfig.Provider)
...
func (s *TfpSanityDualStackProvisioningTestSuite) TestTfpProvisioningSanityDualStack() {
	...
	for _, tt := range tests {
		s.T().Run(tt.name, func(t *testing.T) {
			...
			clusters, _ := provisioning.Provision(s.T(), s.client, s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, perTestTerraformOptions, configMap, newFile, rootBody, file, false, false, true, clusterIDs, customClusterNames, nestedRancherModuleDir)
...
func TestTfpSanityDualStackProvisioningTestSuite(t *testing.T) {
	suite.Run(t, new(TfpSanityDualStackProvisioningTestSuite))
}
