...
func (s *TfpSanityUpgradeRancherTestSuite) TestTfpUpgradeRancher() {
	...
	ranchers.CleanupPreUpgradeClusters(s.T(), s.client, s.terraformOptions, s.terratestConfig, s.cattleConfig, newFile, rootBody, file)

	standardUserClient, newFile, rootBody, file, standardToken, testUser, testPassword = ranchers.SetupResources(s.T(), s.client, s.rancherConfig, s.terratestConfig, s.terraformOptions)

	s.rancherConfig, s.terraformConfig, s.terratestConfig, _ = config.LoadTFPConfigs(s.cattleConfig)
	s.provisionAndVerifyCluster("Sanity_Post_Rancher_Upgrade_", allClusterIDs, newFile, rootBody, file, standardUserClient, standardToken, testUser, testPassword)
...
func (s *TfpSanityUpgradeRancherTestSuite) provisionAndVerifyCluster(name string, clusterIDs []string, newFile *hclwrite.File,
