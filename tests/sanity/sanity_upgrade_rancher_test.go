...
func (s *TfpSanityUpgradeRancherTestSuite) TestTfpUpgradeRancher() {
	...
	s.provisionAndVerifyCluster("Sanity_Pre_Rancher_Upgrade_", clusterIDs, testUser, testPassword, adminToken, standardToken)

	s.cattleConfig, s.terraformOptions, s.upgradeTerraformOptions = infrastructure.UpgradeRancher(s.T(), s.client, s.serverNodeOne, s.session, s.cattleConfig)
	...
	s.provisionAndVerifyCluster("Sanity_Post_Rancher_Upgrade_", clusterIDs, testUser, testPassword, adminToken, standardToken)
...
func (s *TfpSanityUpgradeRancherTestSuite) provisionAndVerifyCluster(name string, clusterIDs []string, testUser, testPassword,
	...
	for _, tt := range tests {
		...
		s.Run((tt.name), func() {
			clusterIDs, customClusterNames = provisioning.Provision(s.T(), s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, s.terraformOptions, configMap, newFile, rootBody, file, false, true, true, customClusterNames)
			provisioning.VerifyClustersState(s.T(), adminClient, clusterIDs)

			if strings.Contains(terraform.Module, clustertypes.WINDOWS) {
				clusterIDs, customClusterNames = provisioning.Provision(s.T(), s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, s.terraformOptions, configMap, newFile, rootBody, file, true, true, true, customClusterNames)
				provisioning.VerifyClustersState(s.T(), adminClient, clusterIDs)
			}
		})
