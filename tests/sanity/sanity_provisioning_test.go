...
func (s *TfpSanityProvisioningTestSuite) TestTfpProvisioningSanity() {
	...
	for _, tt := range tests {
		...
		s.Run((tt.name), func() {
			...
			clusterIDs, customClusterNames := provisioning.Provision(s.T(), s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, s.terraformOptions, configMap, newFile, rootBody, file, false, false, true, customClusterNames)
			provisioning.VerifyClustersState(s.T(), adminClient, clusterIDs)

			if strings.Contains(terraform.Module, clustertypes.WINDOWS) {
				clusterIDs, _ := provisioning.Provision(s.T(), s.standardUserClient, rancher, terraform, terratest, testUser, testPassword, s.terraformOptions, configMap, newFile, rootBody, file, true, true, true, customClusterNames)
				provisioning.VerifyClustersState(s.T(), adminClient, clusterIDs)
			}
		})
