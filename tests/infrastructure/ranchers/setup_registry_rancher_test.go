...
type RegistryRancherTestSuite struct {
	suite.Suite
...
func (i *RegistryRancherTestSuite) TestCreateRegistryRancher() {
	...
	_, keyPath := rancher2.SetKeyPath(keypath.RegistryKeyPath, i.terratestConfig.PathToRepo, i.terraformConfig.Provider)
	terraformOptions := framework.Setup(i.T(), i.terraformConfig, i.terratestConfig, keyPath)
	i.terraformOptions = terraformOptions

	_, _, _, err := registries.CreateMainTF(i.T(), i.terraformOptions, keyPath, i.rancherConfig, i.terraformConfig, i.terratestConfig)
...
func TestRegistryRancherTestSuite(t *testing.T) {
	suite.Run(t, new(RegistryRancherTestSuite))
}
