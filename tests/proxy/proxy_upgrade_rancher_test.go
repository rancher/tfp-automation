...
func (p *TfpProxyUpgradeRancherTestSuite) TestTfpUpgradeProxyRancher() {
	...
	ranchers.CleanupPreUpgradeClusters(p.T(), p.client, p.terraformOptions, p.terratestConfig, p.cattleConfig, newFile, rootBody, file)

	standardUserClient, newFile, rootBody, file, standardToken, testUser, testPassword = ranchers.SetupResources(p.T(), p.client, p.rancherConfig, p.terratestConfig, p.terraformOptions)

	p.rancherConfig, p.terraformConfig, p.terratestConfig, _ = config.LoadTFPConfigs(p.cattleConfig)
	p.provisionAndVerifyCluster("Proxy_Post_Rancher_Upgrade_", allClusterIDs, newFile, rootBody, file, standardUserClient, standardToken, testUser, testPassword)
...
func (p *TfpProxyUpgradeRancherTestSuite) provisionAndVerifyCluster(name string, clusterIDs []string, newFile *hclwrite.File,
