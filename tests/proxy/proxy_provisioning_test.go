...
func (p *TfpProxyProvisioningTestSuite) TestTfpNoProxyProvisioning() {
	...
	for _, tt := range tests {
		p.T().Run(tt.name, func(t *testing.T) {
			...
			clusters, customClusterNames := provisioning.Provision(p.T(), p.client, p.standardUserClient, rancher, terraform, terratest, testUser, testPassword, perTestTerraformOptions, configMap, newFile, rootBody, file, false, false, true, clusterIDs, customClusterNames, nestedRancherModuleDir)
			...
			if strings.Contains(terraform.Module, clustertypes.WINDOWS) {
				clusters, _ = provisioning.Provision(p.T(), p.client, p.standardUserClient, rancher, terraform, terratest, testUser, testPassword, perTestTerraformOptions, configMap, newFile, rootBody, file, true, true, true, clusterIDs, customClusterNames, nestedRancherModuleDir)
				err = provisioningActions.VerifyClusterReady(p.client, clusters[0])
				require.NoError(p.T(), err)
				err = clusterActions.VerifyServiceAccountTokenSecret(p.client, clusters[0].Name)
				require.NoError(p.T(), err)
...
func (p *TfpProxyProvisioningTestSuite) TestTfpProxyProvisioning() {
	...
	for _, tt := range tests {
		p.T().Run(tt.name, func(t *testing.T) {
			...
			clusters, _ := provisioning.Provision(p.T(), p.client, p.standardUserClient, rancher, terraform, terratest, testUser, testPassword, perTestTerraformOptions, configMap, newFile, rootBody, file, false, false, true, clusterIDs, customClusterNames, nestedRancherModuleDir)
			err = provisioningActions.VerifyClusterReady(p.client, clusters[0])
			require.NoError(p.T(), err)
			err = clusterActions.VerifyServiceAccountTokenSecret(p.client, clusters[0].Name)
			require.NoError(p.T(), err)
