...
func (a *TfpAirgapProvisioningTestSuite) TestTfpAirgapProvisioning() {
	...
	for _, tt := range tests {
		...
		a.Run((tt.name), func() {
			...
			clusterIDs, customClusterNames := provisioning.Provision(a.T(), a.standardUserClient, rancher, terraform, terratest, testUser, testPassword, a.terraformOptions, configMap, newFile, rootBody, file, false, false, true, customClusterNames)
			provisioning.VerifyClustersState(a.T(), adminClient, clusterIDs)

			if strings.Contains(terraform.Module, clustertypes.WINDOWS) {
				clusterIDs, _ = provisioning.Provision(a.T(), a.standardUserClient, rancher, terraform, terratest, testUser, testPassword, a.terraformOptions, configMap, newFile, rootBody, file, true, true, true, customClusterNames)
				provisioning.VerifyClustersState(a.T(), adminClient, clusterIDs)
			}
		})
