...
func Resolve(host, adminToken string, insecure bool, pinned string, pinnedByMinor map[string]string) (string, error) {
	...
	if version := strings.TrimPrefix(os.Getenv(ProviderEnvVar), "v"); version != "" {
		...
		logrus.Debugf("Using rancher2 provider version %s, from %s", version, ProviderEnvVar)
	...
	if pinned != "" {
		...
		resolved[host] = version
		logrus.Debugf("Using rancher2 provider version %s, from the config file", version)
	...
	resolved[host] = version
	logrus.Debugf("Using rancher2 provider version %s, resolved for the Rancher under test", version)
