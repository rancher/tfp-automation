...
// CreateImportedNullResourceCopy is a helper function that will create the null_resource for the cluster.
func CreateImportedNullResourceCopy(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig, publicDNS, resourceName, clusterName string) (*hclwrite.Body, *hclwrite.Body) {
	nullResourceBlock := rootBody.AppendNewBlock(defaults.Resource, []string{defaults.NullResource, resourceName})
	nullResourceBlockBody := nullResourceBlock.Body()
...
func createImportedRKE2K3SServer(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig, serverOnePublicDNS, serverOnePrivateIP,
	...
	copyScriptName := clusterName + `_copy_script_` + serverOne
	_, provisionerBlockBody := CreateImportedNullResourceCopy(rootBody, terraformConfig, serverOnePublicDNS, copyScriptName, clusterName)
	...
	nullResourceBlockBody, provisionerBlockBody := CreateImportedNullResourceCopy(rootBody, terraformConfig, serverOnePublicDNS, createClusterName, clusterName)
...
func addImportedRKE2K3SServerNodes(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig, serverOnePrivateIP, serverTwoPublicDNS,
	...
	for i, dns := range dnsList {
		...
		nullResourceBlockBody, provisionerBlockBody := CreateImportedNullResourceCopy(rootBody, terraformConfig, dns, resourceName, clusterName)
		...
		nullResourceBlockBody, provisionerBlockBody = CreateImportedNullResourceCopy(rootBody, terraformConfig, dns, addServer+"_"+resourceName, clusterName)
