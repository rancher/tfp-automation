...
func CreateAWSInstances(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig, terratestConfig *config.TerratestConfig,
	...
	if terraformConfig.MixedArchitecture {
		if strings.Contains(hostnamePrefix, serverOne) {
			configBlockBody.SetAttributeValue(aws.Ami, cty.StringVal(terraformConfig.AWSConfig.ARMAMI))
			configBlockBody.SetAttributeValue(aws.InstanceType, cty.StringVal(terraformConfig.AWSConfig.ARMInstanceType))
		} else {
...
// CreateCustomClusterAWSInstances is a function that will set the AWS instances configurations in the main.tf file for a RKE2/K3s cluster.
func CreateCustomClusterAWSInstances(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig, terratestConfig *config.TerratestConfig,
	hostnamePrefix, ami, instanceType string, count int64) {
	configBlock := rootBody.AppendNewBlock(general.Resource, []string{aws.AwsInstance, hostnamePrefix})
	configBlockBody := configBlock.Body()

	configBlockBody.SetAttributeValue(general.Count, cty.NumberIntVal(count))
	configBlockBody.SetAttributeValue(aws.Ami, cty.StringVal(ami))
	configBlockBody.SetAttributeValue(aws.InstanceType, cty.StringVal(instanceType))
