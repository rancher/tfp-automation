...
func ConfigTF(client *rancher.Client, rancherConfig *rancher.Config, terraformConfig *configuration.TerraformConfig, terratestConfig *configuration.TerratestConfig,
	...
	for i, config := range configMap {
		...
		case strings.Contains(module, clustertypes.RKE1) && !strings.Contains(module, defaults.Custom) && !strings.Contains(module, defaults.Import):
		...
		case (strings.Contains(module, clustertypes.RKE2) || strings.Contains(module, clustertypes.K3S)) && !strings.Contains(module, defaults.Custom) && !strings.Contains(module, defaults.Import):
